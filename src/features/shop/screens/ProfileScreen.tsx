...
import type { ThemePreference } from "@core/theme/types";
...
export default function ProfileScreen() {
  ...
  // theme preference in your store: "light" | "dark" | "system"
  const preference = useThemeStore((s) => s.preference);
  const setPreference = useThemeStore((s) => s.setPreference);

  const isDark =
    preference === "dark" || (preference === "system" && t.mode === "dark");

  const onToggleDark = (next: boolean) => {
    setPreference((next ? "dark" : "light") as ThemePreference);
  };

  const onLogout = async () => {
    try {
      // sign-out with your auth client here if applicable
      ...
      >
        ...
        <View style={styles.profileCard}>
          <View style={styles.avatarWrap}>
            ...
            <View
              style={[styles.editBadge, { backgroundColor: t.colors.text }]}
            >
          ...
          <Text style={[styles.phone, { color: t.colors.textSecondary }]}>
            +1 111 467 378 399
          </Text>
        </View>
        ...
        <View style={styles.menu}>
          ...
          <Row
            icon={
              <Feather name="credit-card" size={18} color={t.colors.text} />
            }
            label="Payment"
          ...
          <Row
            icon={
              <Feather name="help-circle" size={18} color={t.colors.text} />
            }
            label="Help Center"
